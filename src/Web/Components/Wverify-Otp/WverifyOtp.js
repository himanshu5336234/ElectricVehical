     ...
     if(!res.data.error){  //error check for otp
      ...
      if(res.data?.bookData?._id){ //checking for booking
        ...
        if(bookingDetails.cityAvailability==="yes"){
          ...
          navigate('/WShowroom',{state: {bookingDetails:{...bookingDetails,UserId:res.data.user._id}}})
    ...
    className='w-otp-container'>
      ...
      <div className='w-otp-top'>
        ...
        <div className='w-otp-top-otplabel'>
          <h3 className='w-otp-top-heading'>Punch in the OTP</h3>
          <span className='w-otp-top-info'>We’ve sent you one on <br/>+91 {bookingDetails?.phone}</span>
        </div>
      </div>
