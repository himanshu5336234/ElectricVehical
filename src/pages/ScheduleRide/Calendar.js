...
export default function App({setshowdateSlot,setSelectDate,bookingDateUpdate,setShow}) {
...
//function ------------------------on click date
  ...
  const DisblesDate = setShow?.[setShow?.length -1]?.date.split("-")?.reverse()?.join(",")
  return (
    <>
      <Calendar style={{width:"100% !important"}}
      value={dateState}
      onChange={changeDate}
      minDate={new Date()}
