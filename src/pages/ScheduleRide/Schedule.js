...
import InprogressComp from "../../components/LoadingPage/LoadingPage";
import CompleteProgress from "../../components/SuccessfulLoading/SuccessfullLoading"
...
"July", "Aug", "Sept", "Oct", "Nov", "Dec"];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      dispatch(setBooking({...bookingRes.data.data,payment:bookingRes.data.payment}));
  ...
  return (
    <>
    {
      ...
      <MainDiv>
        ...
        <BackgroundDiv>
          <HeadingDiv>
            <img src={backImg} onClick={() => navigate(`/showroomselection`)} />
                ...
                {selectDate ? (
                  <>
                    {dummySlot?.map((i, index) => (
                      <SelectSlotDiv onClick={!availableSlots?.includes(i) ? console.log("testing slots"):() => setTimeSlot(i)}>
                        <SlotDiv
                          style={{
                            border: active === index && availableSlots?.includes(i)  ? "1px solid #000" : "none",
                            color: availableSlots?.includes(i) ? "#000" : "rgba(17, 19, 24, 0.2)"
                          }}
                  ...
                  <SelectSlotDate1>
                    <ConfirmDate>
                      <Img1
                        src={CalImg}
                      />
                      {`${month.filter((month,index)=> index==showdateslot.split("-")[1]-1 )} ${ showdateslot.split("-")[0]},${ showdateslot.split("-")[2]}`}
                    </ConfirmDate>
          ...
          {!timeSlot ? (
            <ButtonDiv disabled={timeSlot}>Schedule It. Can’t Wait</ButtonDiv>
          ) : null}
        </BackgroundDiv>
      </div>:
      loader === 1 ?
