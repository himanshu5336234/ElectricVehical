...
const [searchParams] = useSearchParams()
  const [pressDetailsData, setPressData] = useState({});
 const press_id =searchParams.get("press_id")
console.log()

  useEffect(() => {
    apiServices(press_id);
  }, []);
  ...
  function apiServices(id) {
    ...
    axios
      .post("https://api.obenev.com/get-presses-details", { press_id: id })

      .then((res) => setPressData(res.data.data))
  ...
  return (
    <>
      <MainDiv>
        ...
        <Div>
          <Div2>
            <SubDiv2>{new Date(pressDetailsData.date).toDateString()}</SubDiv2>
